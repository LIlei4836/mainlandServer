...
def saveRedisSorted(data,currsname,time_name):
    ...
    # 只保留有序集合最近2018条
    r.zremrangebyrank("market:" + currsname + ":" + time_name + ":redisSortset3.2.12", 0, -600)
...
def getdata(resolution,symbol,currsname,time_name):
    ...
    while 1:
        try:
            ...
            urlDict = ["http://tvc4.forexpros.com/2b3e7c8c1966d9488b9b637c11017675/", str(time_stamp),"/6/6/28/history?symbol=", str(symbol),"&resolution=",str(resolution),"&from=",
                       str(time_stamp - time_len), "&to=", str(time_stamp)]
            url = ''.join(urlDict)
            # print(url)
...
if __name__ == '__main__':
    ...
    A = {'2':'gbpusd','5':'audusd','7':'usdcad','8':'nzdusd','4':'usdchf','3':'usdjpy','11':'gbpjpy','9':'eurjpy'}
    p = Pool(len(A))
    for symbol in A:
